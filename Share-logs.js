(function(){
  ...
  // --- Основные элементы скролла и таблицы ---
  ...
  // --- Share и Copy Link ---
  function clickShareAndCopy(callback) {
      const shareBtn = document.querySelector('[data-test-subj="shareTopNavButton"]');
      if (!shareBtn) {
      ...
      const waitMenu = setInterval(() => {
          const copyBtn = document.querySelector('[data-test-subj="copyShareUrlButton"]');
          if (copyBtn) {
              clearInterval(waitMenu);

              const toggle = document.querySelector('[data-test-subj="useShortUrl"]');
              ...
              // Закрываем меню кликом по Share
              shareBtn.click();
