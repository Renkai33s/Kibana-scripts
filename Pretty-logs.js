(async () => {
  // --- Система уведомлений (как во втором скрипте) ---
  ...
  try {
    ...
    // 🔒 Жёсткая проверка: должен быть именно выделенный текст, а не просто курсор
    if (!sel || sel.rangeCount === 0 || sel.isCollapsed || !sel.toString().trim()) {
      showError("Логи нвыделены");
      return;
    }

    const allTr = Array.from(document.querySelectorAll("tr"));
    // фильтруем только те строки, которые действительно попали в выделение
