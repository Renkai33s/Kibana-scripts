(async () => {
  ...
  const COL_SEP = " "; // ← разделитель столбцов — один пробел
  ...
  if (!sel || sel.rangeCount === 0) { alert("Нет текстового выделения. Выдели строки таблицы и попробуй снова."); return; }
  ...
  const buildIdxMapFromTable = (tableEl) => {
    ...
    ["time","message.message","message.exception","payload"].forEach(w => {
  ...
  if (!rows.length) { alert("Не удалось найти данные. Убедись, что выделены строки и есть колонки time, message.message, message.exception, payload."); return; }

  // Разделитель между столбцами — один пробел
  const lines = rows
    .map(r => r.join(COL_SEP).replace(/[ \t]+$/g, "")) // trimEnd без трогания внутренних \n
  ...
  alert(ok ? `Скопировано ${lines.length} строк (разделитель: пробел).` : "Не получилось скопировать в буфер. Результат выведен в консоль.");
})();
